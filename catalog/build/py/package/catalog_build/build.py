    ...
    for batch_index, batch_url in enumerate(get_batched_ncbi_urls(get_base_url, items)):
        results.extend(
            get_paginated_ncbi_results(
                batch_url, f"{query_description} batch {batch_index + 1}"
            )
        )
    return results
...
def get_species_df(species_info, taxonomic_group_sets, taxonomic_levels):
    ...
    return pd.DataFrame(
        [
            get_species_row(info, taxonomic_group_sets, taxonomic_levels)
...
def get_species_tree(taxonomy_ids, taxonomic_levels, species_info=None):
    ...
    fetch_taxa_info_in_batches(
        tax_ids, taxon_name_map, taxon_rank_map, "missing parent taxa"
    )
...
def fetch_taxa_info_in_batches(
    tax_ids, taxon_name_map, taxon_rank_map, description="taxa"
):
    """
    ...
    for i in range(0, len(missing_tax_ids), batch_size):
        ...
        print(
            f"Fetching batch {i // batch_size + 1} of {(len(missing_tax_ids) + batch_size - 1) // batch_size} ({len(batch)} taxa)"
        )
...
def build_files(
    ...
    species_df = get_species_df(
        species_info, taxonomic_group_sets, taxonomic_levels_for_tree
    )
