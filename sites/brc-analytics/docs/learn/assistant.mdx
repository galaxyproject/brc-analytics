...
During the beta we also keep a copy of each exchange -- what you asked, what the assistant answered, and which catalog lookups it made -- so we can see where it helps and where it falls short. Records are deleted automatically once they pass the retention window, which is shown at the bottom of the assistant page (90 days by default). Only the maintainers can read them, they aren't published or shared, and nothing is recorded about you beyond your account if you're signed in. Routine database backups can hold a copy for a while after that, and those age out on their own schedule. This is the same reason the point above matters: don't type anything into the assistant you wouldn't want kept.
