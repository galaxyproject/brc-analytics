...
5. **Start from a Logan search.** A finished [Logan search](/logan-search) has an **Ask the assistant** button. It opens a conversation that already knows the cohort -- how many runs matched, the top organisms, platforms and layouts over the whole match set -- with the organism prefilled when one dominates. From there you can ask what the cohort is or set up an analysis on the top runs. The assistant describes the search from the whole-match-set counts, never from the handful of top hits it lists.
...
During the beta we may also keep a copy of each exchange -- what you asked, what the assistant answered, and which catalog lookups it made -- so we can see where it helps and where it falls short. Whether that's on, and how long those records are kept before they're deleted automatically, is stated at the bottom of the assistant page; on this site it's 90 days. If the notice isn't there, nothing is being kept beyond the short-lived storage described above. Only the maintainers can read them, they aren't published or shared, and nothing is recorded about you beyond your account if you're signed in. Routine database backups can hold a copy for a while after that, and those age out on their own schedule. This is the same reason the point above matters: don't type anything into the assistant you wouldn't want kept.
